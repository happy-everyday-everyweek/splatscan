...
void FeatureTracker::detect(const uint8_t* gray, int width, int height, int stride,
    ...
    // 格子边长必须跟着图像尺寸收缩：训练图只有一百多像素时，固定 24 像素的格子
    // 会把整张图切成十几个格子，特征点数量根本不够跟踪用。
    const int smallestSide = std::min(width, height);
    const int cell = std::max(6, std::min(minDistance * 2, smallestSide / 8));
