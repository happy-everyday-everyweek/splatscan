...
// 瓦片尺寸统一来自 gs/tile_builder.h 的 kTileSize，这里不再重复定义。
...
bool VulkanRenderer::initialize(ANativeWindow* window, int32_t renderWidth, int32_t renderHeight) {
    ...
    if (!createSetLayouts()) return false;
...
bool VulkanRenderer::createSetLayouts() {
...
bool VulkanRenderer::createDescriptorResources() {
    // 改分辨率会重建存储图像，所以池与集合必须一并重建；布局与管线布局只建一次，重建后继续复用。
    if (descriptorPool_ != VK_NULL_HANDLE) {
        vkDestroyDescriptorPool(device_, descriptorPool_, nullptr);
        descriptorPool_ = VK_NULL_HANDLE;
    }
    computeSet_ = VK_NULL_HANDLE;
    graphicsSet_ = VK_NULL_HANDLE;
...
bool VulkanRenderer::renderFrame(const TileBundle& bundle) {
    ...
    // 等比缩放：模型图比窗口宽就上下留边，反之左右留边，画面不变形。
    const float imageAspect =
        static_cast<float>(renderWidth_) / static_cast<float>(std::max(1, renderHeight_));
    ...
    if (imageAspect > screenAspect) {
        uvScaleY = screenAspect / imageAspect;
    } else {
        uvScaleX = imageAspect / screenAspect;
    }
    const float blitConstants[4] = {uvScaleX, uvScaleY, 0.0f, 0.0f};
    vkCmdPushConstants(commandBuffer_, graphicsPipelineLayout_, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                       sizeof(blitConstants), blitConstants);
...
void VulkanRenderer::setRenderSize(int32_t renderWidth, int32_t renderHeight) {
    ...
    if (!createStorageImage()) {
        SPLATSCAN_LOGE("重建存储图像失败，渲染暂停");
        ready_ = false;
        return;
    }
    if (!createDescriptorResources()) {
        SPLATSCAN_LOGE("重建描述符资源失败，渲染暂停");
        ready_ = false;
    }
}

void VulkanRenderer::recreateSwapchain() {
    if (!ready_ || swapchain_ == VK_NULL_HANDLE) return;
    vkDeviceWaitIdle(device_);
    ...
    if (!createSwapchain()) {
        SPLATSCAN_LOGE("重建交换链失败");
        ready_ = false;
        return;
    }
    // 表面格式变了的话，渲染通道与图形管线都得重建：它们把格式烘进了创建参数。
    if (swapchainFormat_ != previousFormat) {
        if (renderPass_ != VK_NULL_HANDLE) {
            vkDestroyRenderPass(device_, renderPass_, nullptr);
            renderPass_ = VK_NULL_HANDLE;
        }
        if (!createRenderPass() || !createBlitPipeline()) {
            SPLATSCAN_LOGE("格式变化后重建渲染通道或图形管线失败");
            ready_ = false;
            return;
        }
    }
    if (!createFramebuffers()) {
        SPLATSCAN_LOGE("重建帧缓冲失败");
        ready_ = false;
    }
}
