...
std::mutex gMutex;
std::unordered_map<int64_t, std::unique_ptr<splatscan::ScanEngine>> gEngines;
int64_t gNextHandle = 1;

splatscan::ScanEngine* findEngine(int64_t handle) {
...
std::string coreVersionString() { return std::string("splatscan-core/0.2.0"); }
...
Java_com_splatscan_app_native_SplatCore_nativeCreate(JNIEnv* /*env*/, jclass /*clazz*/,
    ...
    auto engine = std::make_unique<splatscan::ScanEngine>(config);
    std::lock_guard<std::mutex> lock(gMutex);
...
Java_com_splatscan_app_native_SplatCore_nativeStatus(JNIEnv* env, jclass /*clazz*/, jlong handle) {
    ...
    if (auto* engine = findEngine(handle)) {
        const splatscan::EngineStatus& status = engine->status();
        values[0] = static_cast<jfloat>(status.acceptedFrames);
        values[1] = static_cast<jfloat>(status.rejectedFrames);
        values[2] = status.stability;
        values[3] = status.coverage;
        values[4] = static_cast<jfloat>(status.gaussianCount);
        values[5] = static_cast<jfloat>(status.rounds);
        values[6] = static_cast<jfloat>(status.trackedFeatures);
        values[7] = status.residual;
        values[8] = status.modelMemoryMb;
        values[9] = status.memoryCapped ? 1.0f : 0.0f;
        values[10] = status.initialized ? 1.0f : 0.0f;
        values[11] = status.lastRingX;
        values[12] = status.lastRingY;
        values[13] = status.vulkanActive ? 1.0f : 0.0f;
    }
