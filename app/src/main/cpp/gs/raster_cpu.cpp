...
void CpuRasterizer::project(const GaussianParams& model, const Pose& pose,
    ...
    boundsMinX_ = camera.width;
    boundsMaxX_ = -1;
    boundsMinY_ = camera.height;
    boundsMaxY_ = -1;
    ...
    for (int32_t i = 0; i < model.count; ++i) {
        ...
        const int32_t left = std::max(0, static_cast<int32_t>(item.centerX - radius));
        const int32_t right = std::min(camera.width - 1, static_cast<int32_t>(item.centerX + radius));
        const int32_t top = std::max(0, static_cast<int32_t>(item.centerY - radius));
        const int32_t bottom = std::min(camera.height - 1, static_cast<int32_t>(item.centerY + radius));
        if (left < boundsMinX_) boundsMinX_ = left;
        if (right > boundsMaxX_) boundsMaxX_ = right;
        if (top < boundsMinY_) boundsMinY_ = top;
        if (bottom > boundsMaxY_) boundsMaxY_ = bottom;
    }
...
void CpuRasterizer::render(const GaussianParams& model, const Pose& pose,
    ...
    if (visible.empty() || boundsMaxX_ < boundsMinX_ || boundsMaxY_ < boundsMinY_) return;

    for (int32_t y = boundsMinY_; y <= boundsMaxY_; ++y) {
        const float py = static_cast<float>(y) + 0.5f;
        for (int32_t x = boundsMinX_; x <= boundsMaxX_; ++x) {
...
float CpuRasterizer::trainStep(GaussianParams& model, const Pose& pose,
    ...
    std::vector<VisibleGaussian> visible;
    project(model, pose, camera, visible);
    if (visible.empty() || boundsMaxX_ < boundsMinX_ || boundsMaxY_ < boundsMinY_) return 1.0f;
