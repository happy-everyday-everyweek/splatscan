...
void ScanEngine::downsample(const uint8_t* y, int32_t width, int32_t height, int32_t yStride,
    ...
    for (int32_t oy = 0; oy < outHeight; ++oy) {
        const int32_t y0 = std::min(height - 1, static_cast<int32_t>(oy * scaleY));
        const int32_t y1 = std::min(height, std::max(y0 + 1, static_cast<int32_t>((oy + 1) * scaleY)));
        const int32_t stepY = std::max(1, (y1 - y0) / 4);
        for (int32_t ox = 0; ox < outWidth; ++ox) {
            const int32_t x0 = std::min(width - 1, static_cast<int32_t>(ox * scaleX));
            const int32_t x1 = std::min(width, std::max(x0 + 1, static_cast<int32_t>((ox + 1) * scaleX)));
            const int32_t stepX = std::max(1, (x1 - x0) / 4);
            ...
            float vSum = 0.0f;
            int32_t samples = 0;
            for (int32_t sy = y0; sy < y1; sy += stepY) {
                for (int32_t sx = x0; sx < x1; sx += stepX) {
...
void ScanEngine::initializeFromFrame(const std::vector<uint8_t>& rgb) {
    ...
    for (int32_t y = step / 2; y < config_.trainHeight; y += step) {
        for (int32_t x = step / 2; x < config_.trainWidth; x += step) {
            ...
            const Vec3 world = worldFromCamera * Vec3{px, py, kInitialDepth};
