...
void ScanEngine::submitImu(float ax, float ay, float az, float gx, float gy, float gz,
    ...
    // 重力方向用低通估计，水平方向的比力用于短时速度估计
    gravity_ = gravity_ * 0.95f + acceleration * 0.05f;

    float dt = 0.0f;
    if (lastImuNs_ != 0 && timestampNs > lastImuNs_) {
    ...
    if (dt > 0.0f) {
        ...
        const Vec3 linear = acceleration - gravity_;
        velocity_ += linear * dt;
        // 速度用衰减项约束，避免二次积分发散
        velocity_ = velocity_ * 0.98f;
        if (state_ == EngineState::Scanning) {
            pose_.translation += velocity_ * dt;
        }
    }
...
void ScanEngine::initializeFromFrame(const std::vector<uint8_t>& rgb) {
    ...
    for (int32_t y = step / 2; y < config_.trainHeight; y += step) {
        for (int32_t x = step / 2; x < config_.trainWidth; x += step) {
            ...
            const Vec3 color{static_cast<float>(rgb[index]) / 255.0f - 0.5f,
                             static_cast<float>(rgb[index + 1]) / 255.0f - 0.5f,
                             static_cast<float>(rgb[index + 2]) / 255.0f - 0.5f};
            model_.add({px, py, kInitialDepth}, std::log(scale), std::log(scale), color, 2.0f);
        }
    }
    pose_ = Pose{};
    pose_.rotation = rotation_;
    pose_.translation = {0.0f, 0.0f, 0.0f};
...
void ScanEngine::processFrame(const std::vector<uint8_t>& rgb, const std::vector<uint8_t>& gray) {
    ...
    const float scaleHint = 0.02f;
    const PoseSolution solution = PoseSolver::solveTranslation(tracks_, camera_, rotation_, scaleHint);

    status_.trackedFeatures = tracked;
    if (solution.valid && tracked >= 20) {
        pose_.rotation = solution.rotation;
        // 平移叠加，保持相机在世界中的连续轨迹
        pose_.translation += solution.translation;
