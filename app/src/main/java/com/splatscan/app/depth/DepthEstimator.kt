...
/**
 * 单目深度估计：Depth-Anything-V2-Small（fp32 权重，Apache-2.0）。
 *
...
class DepthEstimator(private val context: Context) {
    ...
    /** 新一次扫描允许重试：清掉上一次的失败标记（已装载的会话继续复用）。 */
    ...
    private fun stage(message: String) {
        runCatching { onStage?.invoke(message) }
    }
    ...
    /**
     * 模型以文件形式加载：94MB 的权重留在磁盘上，避免在 Java 堆里再放一份副本
     * （堆里放 94MB 再交给运行时解析，是最容易被 OOM 打断的一步）。
     */
    private fun ensureModelFile(): String {
        ...
        if (target.exists() && target.length() > MIN_MODEL_BYTES) {
            stage("模型文件已在本地（${target.length() / (1024 * 1024)} MB），跳过写入")
            return target.absolutePath
        }
        if (target.exists()) {
            // 上次写入可能被中断，留下不完整的文件；删掉重写，避免装载损坏模型。
            stage("检测到不完整的模型缓存（${target.length() / (1024 * 1024)} MB），重新写入")
            target.delete()
        }
        val started = System.currentTimeMillis()
        stage("首次使用：正在把模型写入应用目录（约 95 MB）…")
        context.assets.open(MODEL_ASSET).use { input ->
            FileOutputStream(target).use { output -> input.copyTo(output, 1 shl 20) }
        }
        stage("模型写入完成（用时 ${System.currentTimeMillis() - started} ms，${target.length() / (1024 * 1024)} MB）")
        return target.absolutePath
    }
    ...
    private fun ensureSession(): OrtEnvironment? {
        ...
        return try {
            ...
            val started = System.currentTimeMillis()
            stage("正在创建推理会话（约 95 MB 模型）…")
            val env = OrtEnvironment.getEnvironment()
            val options = OrtSession.SessionOptions().apply {
                setIntraOpNumThreads(Runtime.getRuntime().availableProcessors().coerceIn(2, 6))
                setOptimizationLevel(OrtSession.SessionOptions.OptLevel.ALL_OPT)
            }
            val created = env.createSession(path, options)
            stage("推理会话就绪（用时 ${System.currentTimeMillis() - started} ms）")
            environment = env
            session = created
    ...
    companion object {
        ...
        /** 小于这个字节数的缓存文件视为写入不完整（fp32 模型约 99MB）。 */
        private const val MIN_MODEL_BYTES = 90_000_000L

        /**
         * fp32 权重：只用标准算子（Conv/MatMul/Gemm/Resize 等），Android 版运行时必然支持。
         * int8 动态量化版在 Android 上会因 ConvInteger 缺实现而直接失败；4 位版依赖贡献域算子，
         * 同样不适合拿来赌。
         */
        private const val MODEL_ASSET = "depth_anything_v2_small_fp32.onnx"
    }
}
