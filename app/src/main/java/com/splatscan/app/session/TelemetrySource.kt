...
class TelemetrySource(private val context: Context) {
    ...
    /** 原生渲染线程报上来的帧率：Vulkan 直出时它是唯一可信的帧率来源。 */
    fun reportRenderFps(fps: Float) {
        if (fps > 0.5f && fps.isFinite()) {
            fpsAccumulator = fps
            renderFps = fps
        }
    }

    fun snapshot(tracking: TrackingTelemetry, model: ModelTelemetry, stepMillis: Float = 0f): TelemetrySnapshot {
