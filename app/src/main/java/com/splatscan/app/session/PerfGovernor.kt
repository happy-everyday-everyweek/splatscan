...
object PerfGovernor {
    ...
    private const val MIN_IMAGE_SIDE = 320
    ...
    fun initial(totalMemoryMb: Int, charging: Boolean): ScanSpec {
        ...
        return ScanSpec(
            ...
            iterationsPerBatch = if (charging) 8 else 5,
            maxGaussians = (memoryCeiling / 2).coerceAtLeast(MIN_GAUSSIANS),
            trainingImageLongSide = 640,
        )
    }
    ...
    fun next(previous: ScanSpec, snapshot: TelemetrySnapshot, dtSeconds: Float): ScanSpec {
        ...
        if (fps in 0.1f..MIN_FPS) {
            iterations = (iterations - 2).coerceAtLeast(2)
            keyframe = (keyframe + 40).coerceAtMost(500)
            imageSide = (imageSide - 64).coerceAtLeast(MIN_IMAGE_SIDE)
        } else if (fps in MIN_FPS..TARGET_FPS) {
            iterations = (iterations - 1).coerceAtLeast(2)
        } else if (fps > TARGET_FPS * 1.35f) {
            // 有余量就往前试探，每次只走一小步。
            iterations = (iterations + 1).coerceAtMost(32)
        ...
        iterations = iterations.coerceIn(2, 32)
