...
class ScanSession(private val context: Context, private val store: ScanStore) {
    ...
    private var depthStartedAt = 0L
    private var depthWatchdogScheduled = false
    private val depthWatchdogIntervalMs = 10_000L
    private val depthWatchdog = object : Runnable {
        override fun run() {
            if (!depthPending) {
                depthWatchdogScheduled = false
                return
            }
            val seconds = (System.currentTimeMillis() - depthStartedAt) / 1000
            appendLog("深度估计：仍在进行中（已 ${seconds} 秒）")
            mainHandler.postDelayed(this, depthWatchdogIntervalMs)
        }
    }
    ...
    fun start() {
        ...
        // 每次新扫描都重新估一次深度：场景变了，旧的深度图不能复用；失败状态一并清掉，允许重试。
        depthRequested = false
        depthEstimator?.resetFailureState()
     ...
     * 每个阶段（模型写入 / 会话装载 / 推理 / 结果）都会写一条日志，
     * 每 10 秒还有一条心跳：万一哪一步卡住，日志能直接显示停在哪一步。
     */
    private fun maybeStartDepthEstimation(packet: FramePacket) {
        ...
        estimator.onStage = { message -> mainHandler.post { appendLog(message) } }
        val heapBefore = Runtime.getRuntime().let { it.totalMemory() - it.freeMemory() }

        mainHandler.post {
            appendLog("深度估计：已请求（后台执行）")
            depthPending = true
            depthStartedAt = System.currentTimeMillis()
            if (!depthWatchdogScheduled) {
                depthWatchdogScheduled = true
                mainHandler.postDelayed(depthWatchdog, depthWatchdogIntervalMs)
            }
        }
        ...
        Thread {
            ...
            val size = estimator.inputSize
            val summary = estimator.modelSummary
            val error = estimator.lastError

            mainHandler.post {
                depthPending = false
                if (handle == 0L) {
                    appendLog("深度估计已完成，但引擎已释放，结果丢弃")
                    return@post
                }
                if (summary.isNotEmpty()) appendLog("深度模型 $summary")
                if (depth != null) {
                    val ok = SplatCore.submitDepth(handle, size, size, depth)
                    appendLog(
                        "深度估计完成 ${elapsedMs}ms，${size}×${size}，堆内存 +${deltaMb}MB，" +
                            if (ok) "已按深度铺初模" else "提交失败，退回平面初模",
                    )
                } else {
                    // 立刻告诉原生侧别再等了，直接退回平面初模。
                    SplatCore.depthUnavailable(handle)
                    appendLog("深度估计失败（${elapsedMs}ms）：${error ?: "未知原因"}，退回平面初模")
                }
            }
        }.start()
    }
