...
class ScanSession(private val context: Context, private val store: ScanStore) {
    ...
    fun tick() {
        ...
        val snapshot = telemetrySource.snapshot(tracking, model)
        ...
        if (phase == ScanPhase.Scanning) {
            ...
            if (now - lastDiagnosticMillis >= DIAGNOSTIC_INTERVAL_MS) {
                ...
                appendLog(
                    "诊断 接受${status.acceptedFrames}/拒绝${status.rejectedFrames}" +
    ...
    fun telemetryFps(): Int = lastSnapshot.device.renderFps.toInt()
